...
def domain_redirected(url):
    p = subprocess.run("curl -I google.com", shell=True, capture_output=True, text=True)
    headers = p.stdout.splitlines()  # Split the output into lines
    location = headers[1]
...
def abnormal_url(domain, url):
    ...
    if re.match(pattern, hostname) and not match:
        return -1
    else:
        return 0
...
def statistical_report(url, hostname):
    ...
    legit_url_match = re.search(
        r'.com|.org|.net|.edu|.gov|.mil|.info|.biz|.name|.pro|.coop|.mobi|.travel|.us|.uk|.ca|.au|.de|.fr|.jp|.cn|.ru|.br|.in', url)
    ...
    if ip_match:
        checked_ip = 0
    if((checked_ip and checked_url) ==0):
        return -1
    else:
        return 1
...
def main(url):
    with open(LOCALHOST_PATH + DIRECTORY_NAME + '/markup.txt', 'r', encoding="utf8") as file:
...
# python features_extraction.py <url>
